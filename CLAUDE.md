...
Railway deploys from the `release` branch, NOT `main`. After pushing changes to `main`, merge to `release` and push to deploy. galton-stream and galton-monitor are separate Railway services in the same repo with watch paths so they deploy independently.
